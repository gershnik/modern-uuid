...
void muuid_SHA1Update(MUUID_SHA1_CTX *context, const unsigned char *data, uint32_t len)
{
	...
	if ((j + len) > 63) {
		...
		muuid_SHA1Transform(context->state, context->buffer);
		for (; i + 63 < len; i += 64) {
			muuid_SHA1Transform(context->state, &data[i]);
		}
		j = 0;
	} else
		i = 0;
...
void muuid_SHA1Final(unsigned char digest[20], MUUID_SHA1_CTX *context)
{
	unsigned i;
...
#endif
	c = 0200;
	muuid_SHA1Update(context, &c, 1);
	while ((context->count[0] & 504) != 448) {
		c = 0000;
		muuid_SHA1Update(context, &c, 1);
	}
	muuid_SHA1Update(context, finalcount, 8);	/* Should cause a SHA1Transform() */
	for (i = 0; i < 20; i++) {
...
void muuid_SHA1(char *hash_out, const char *str, unsigned len)
{
	...
	for (ii = 0; ii < len; ii += 1)
		muuid_SHA1Update(&ctx, (const unsigned char *)str + ii, 1);
	muuid_SHA1Final((unsigned char *)hash_out, &ctx);
	hash_out[20] = '\0';
}
