...
static void muuid_keccak_init(muuid_sha3_ctx* ctx, unsigned bits)
{
	/* NB: The Keccak capacity parameter = bits * 2 */
	unsigned rate = 1600 - bits * 2;

	memset(ctx, 0, sizeof(muuid_sha3_ctx));
...
static void muuid_sha3_permutation(uint64_t* state)
{
	int round;
 ...
 */
static void muuid_sha3_process_block(uint64_t hash[25], const uint64_t* block, size_t block_size)
{
	/* expanded loop */
...
void muuid_sha3_update(muuid_sha3_ctx* ctx, const unsigned char* msg, size_t size)
{
	...
	if (index) {
		...
		muuid_sha3_process_block(ctx->hash, ctx->message, block_size);
		msg  += left;
		size -= left;
	}
	while (size >= block_size) {
		...
		muuid_sha3_process_block(ctx->hash, aligned_message_block, block_size);
		msg  += block_size;
		size -= block_size;
	}
	if (size) {
 ...
 * @param result calculated hash in binary form
 */
void muuid_keccak_final(muuid_sha3_ctx* ctx, unsigned char* result)
{
	size_t digest_length = 100 - ctx->block_size / 2;
	const size_t block_size = ctx->block_size;
